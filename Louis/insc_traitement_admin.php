...
if ($check_result->num_rows > 0) {
    ...
    $_SESSION['notificationinscadmin'] = "Le Nom d'utilisateur ou le Mail existe déjà !";
    // Redirection vers la page de connexion
    header("Location: Page de connexion.php");
} else {
    // Le pseudo et le mail sont uniques, donc procéder à l'insertion
    $insert_query = "INSERT INTO user (nomUser, pseudoUser, mailUser, mdpUser, statutUser) VALUES ('$name', '$username', '$mail', '$password', 'admin')";

    if ($mysqli->query($insert_query) === TRUE) { //Exécution de la ligne SQL et vérification du résultat
        // Démarrage de la session
        session_start();
        // Envoie du message
        $_SESSION['notificationinscadmin'] = "Administrateur inscrit avec succès !";
        // Redirection vers la page de connexion
        ...
        session_start();
        // Envoie du message 
        $_SESSION['notificationinscadmin'] = "Erreur lors de l'inscription !";
        // Redirection vers la page de connexion
        header("Location: Page Admin.php");
    }
}
